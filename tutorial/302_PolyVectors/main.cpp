...
//I modified the code to comply with batch processing.

Eigen::VectorXi constFaces;
directional::TriMesh mesh;
directional::IntrinsicFaceTangentBundle ftb;
directional::CartesianField pvFieldHard, pvFieldSoft, rawField, constraintsField;
Eigen::MatrixXd constVectors;
Eigen::VectorXd alignWeights;
...
int main(int argc, char** argv)
{
    ...
    std::cout <<
        " [EXE] [input_obj_path] [output_4_RoSy_cross_field_path] [Optimal-if True powerFieldHard, if False powerFieldSoft, default=True]" << std::endl;

    // Load mesh
    directional::readOBJ(argv[1], mesh);
    ftb.init(mesh);
    pvFieldHard.init(ftb, directional::fieldTypeEnum::POLYVECTOR_FIELD, N);
    pvFieldSoft.init(ftb, directional::fieldTypeEnum::POLYVECTOR_FIELD, N);
    ...
    directional::polyvector_to_raw((viewFieldHard ? pvFieldHard : pvFieldSoft), rawField, N % 2 == 0);

    if (directional::write_raw_field(argv[2], rawField))
        std::cout << "Saved raw field" << std::endl;
    else
        std::cout << "Unable to save raw field. " << std::endl;
